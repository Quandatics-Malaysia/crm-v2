...
test("staging upgrades retained env through a protected trust-set secret before Compose starts", () => {
  ...
  const quality = readFileSync(resolve(workflows, "quality.yml"), "utf8")
  assert.match(quality, /publish-staging-images:/)
  assert.match(quality, /github\.event_name == 'push'/)
  assert.match(quality, /packages:\s*write/)
  assert.match(quality, /sha-\$\{\{ env\.DEPLOY_SHA \}\}/)
