      ...
      <PageBody>
        <PageHeader title={opp.name}>
          <Button
            ...
            render={
              <Link
                href={`/quotations/new?opportunityId=${opp.id}`}
              />
            }
          >
            <FileTextIcon />
          ...
          <RecordSoDialog
            opportunityId={opp.id}
            currentSoNumber={opp.soNumber}
            trigger={
              <Button variant={soRequiredWarning ? "default" : "outline"}>
                {hasSoNumber ? "Update SO" : "Record SO"}
              </Button>
            }
          />
        </PageHeader>
        ...
        <Card>
          <CardContent className="flex flex-wrap items-center gap-x-8 gap-y-4 py-5">
            ...
            <div className="grid gap-1">
              ...
              <span className="flex items-baseline gap-1.5">
                <span className="text-sm font-semibold tabular-nums">
                  {opp.amount ? formatMoney(opp.amount, opp.currency) : "—"}
                </span>
            ...
            <div className="grid gap-1">
              <span className="text-xs text-muted-foreground">SO Number</span>
              {hasSoNumber ? (
                <span className="text-sm font-semibold tabular-nums">
                  {soNumber}
                </span>
                ...
                <Badge variant="destructive">
                  SO number + signed PO/quotation required
                </Badge>
              ) : (
                  ...
                  <TableBody>
                    {detail.quotations.map((q) => (
                      <TableRow key={q.id}>
                        ...
                        <TableCell>
                          <Badge
                            variant={quoteStatusVariant[q.status] ?? "secondary"}
        ...
        <Card>
          ...
          <CardContent>
            {documents.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                No documents across this funnel, its quotations, or approvals
                yet.
              </p>
            ) : (
        ...
        <div>
          <Button variant="outline" nativeButton={false} render={<Link href="/funnel" />}>
            Back to funnel
          </Button>
        </div>
      </PageBody>
