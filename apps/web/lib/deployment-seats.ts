    ...
    for (const entry of input.entries) {
      if (entry.kind === "active") {
        const rows = await tx.execute(sql`select * from activate_deployment_membership(
          ...
          ${entry.roleId}, ${entry.tierLevel}, null, ${input.actor.userId},
          ${input.actor.memberId ?? null}, false, ${now.toISOString()}::timestamp with time zone
        ...
        const rows = await tx.execute(sql`select * from reserve_deployment_invitation(
          ...
          ${entry.tierLevel}, ${input.actor.memberId ?? null}, ${input.actor.userId},
          ${input.actor.memberId ?? null}, ${expiresAt.toISOString()}::timestamp with time zone,
...
export async function activateMembership(input: {
  ...
  return db.transaction(async (tx) => {
    ...
    const rows = await tx.execute(sql`select * from activate_deployment_membership(
      ...
      ${input.tierLevel ?? 0}, null, ${input.actor.userId}, ${input.actor.memberId ?? null},
      ${input.bootstrap ?? false}, ${now.toISOString()}::timestamp with time zone
    )`) as unknown as MembershipDecisionRow[]
    const row = rows[0]
    const result = decisionResult(rows[0])
    assertAllowed(result)
...
export async function consumeInvitation(input: {
  ...
  return db.transaction(async (tx) => {
    const proposedMemberId = input.memberId ?? crypto.randomUUID()
    const rows = await tx.execute(sql`select * from activate_deployment_membership(
      ${input.tenantId}, ${input.userId}, ${proposedMemberId}, null, 0,
      ${input.invitationId}::uuid, ${input.actor?.userId ?? input.userId},
      ${input.actor?.memberId ?? null}, false, ${now.toISOString()}::timestamp with time zone
    )`) as unknown as MembershipDecisionRow[]
    const row = rows[0]
    const result = decisionResult(rows[0])
...
export async function disableOrRemoveMembership(input: {
  ...
  return db.transaction(async (tx) => {
    const rows = await tx.execute(sql`select * from change_deployment_membership(
      ...
      ${input.actor.memberId ?? null}, ${now.toISOString()}::timestamp with time zone
