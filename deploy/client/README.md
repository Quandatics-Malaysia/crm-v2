...
Deployment records use schema version 4. Besides prior digests and release identity, the protected record contains release channel, deployment timestamp, prior runtime URLs, application/agent/migration versions, trust set, database/application/auth credentials, Microsoft integration values, demo settings, backup transport target, loopback gateway/database ports, all service memory limits, and application/database health retry timing needed for an exact rollback. Version 3 records upgrade safely as stable releases using their recorded epoch timestamp. These settings are validated before pull, restored before any rollback recreation, and used to derive the old loopback health URL so host Nginx continues to reach the restored gateway. The one-time installation token is intentionally never recorded; registered agent identity lives in the private `agent-state` volume. The record therefore has the same secret sensitivity as `.env`: retain owner-only `0600` permissions, include it in protected backup handling, and never copy it into tickets or logs.
