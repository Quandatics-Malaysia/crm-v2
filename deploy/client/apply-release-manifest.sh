...
jq -e --arg tag "$expected_tag" '
  ...
  .workflow_identity == ("https://github.com/Quandatics-Malaysia/crm-v2/.github/workflows/release-images.yml@refs/tags/" + $tag) and
...
validate_image() {
  value=$1
  repository=$2
  printf '%s\n' "$value" | grep -Eq "^ghcr.io/quandatics-malaysia/${repository}@sha256:[0-9a-f]{64}$" ||
